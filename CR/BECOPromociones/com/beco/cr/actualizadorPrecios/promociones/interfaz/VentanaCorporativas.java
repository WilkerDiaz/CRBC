...
public class VentanaCorporativas extends JDialog implements ComponentListener, KeyListener, MouseListener, EventListener {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(VentanaCorporativas.class);  //  @jve:decl-index=0:
	...
	private Vector<PromocionExt> corporativasActivas; 
	...
	* Fecha: agosto 2011
	*/
	public VentanaCorporativas() {
		
		super(MensajesVentanas.ventanaActiva);

		initialize();
		//TODO
		this.corporativasActivas = PromocionExt.getCorporativasActivas();
		Iterator<PromocionExt> i = corporativasActivas.iterator();
		while (i.hasNext())	{
	...
	private javax.swing.JLabel getJLabel1() {
		...
		if(jLabel1 == null) {
			...
			jLabel1.setText("Promociones corporativas");
	 ...
	 * **/
	private void aceptarCorporativa(){
		try{
			Venta venta = CR.meVenta.getVenta();
			Apartado apartado = CR.meServ.getApartado();
			PromocionExt p = (PromocionExt)this.corporativasActivas.get(jComboBox.getSelectedIndex());
			if(venta!=null){
				venta.setDescuentoCorporativoAplicado(p.getPorcentajeDescuento());
				venta.setCodDescuentoCorporativo(p.getCodPromocion());
				venta.actualizarPreciosDetalle(venta.isAplicaDesctoEmpleado());
			} else if(apartado!=null){
				apartado.setDescuentoCorporativoAplicado(p.getPorcentajeDescuento());
				apartado.setCodDescuentoCorporativo(p.getCodPromocion());
				apartado.actualizarPreciosDetallePromociones();
			}
		} catch(Exception e) {
			e.printStackTrace();
			logger.debug("aceptarCorporativa()",e);
		}
	}
}
