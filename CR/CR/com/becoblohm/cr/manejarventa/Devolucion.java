 ...
 * Fecha       : 11-09-2006
 * Analista    : yzambrano
 * Descripción : Devoluciones en espera
 * =============================================================================
 */
	...
	*/
	...
	public void finalizarTransaccion() throws ConexionExcepcion, XmlExcepcion, FuncionExcepcion, BaseDeDatosExcepcion, PrinterNotConnectedException {
		...
		if (vuelto > 0){
			...
			if (disponible > 0){ //Había algún pago con condicional en la venta original
				//Verificar saldo condicional despues de alguna devolución anterior
				...
				// Compara el monto disponible en condicional con el monto de la devolución
		...
		// Actualizo pagos de la devolución
		// Elimino los pagos asociados a la devolución
		...
		if (vuelto > 0) {
			...
			do {
				...
				try {
					CR.meServ.realizarVentaBR("devolución", vuelto, CR.meVenta.getDevolucion().getCliente(), false);
					reintentar = false;
				} catch (ExcepcionCr e) {
