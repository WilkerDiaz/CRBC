...
public class SavedTransaccion extends Transaccion {
	...
	public SavedTransaccion(int tda, Date fechaT, int caja, int numTransaccion, boolean local) throws BaseDeDatosExcepcion, ConexionExcepcion {
		...
		Vector<Object> cabecera = BaseDeDatosVenta.obtenerTransaccion(tda, fechaSQL, caja, numTransaccion, local); 
		this.local = local;
	...
	* Fecha: agosto 2011
	*/
	public static Vector<SavedTransaccion> getTransacciones(int tda, int caja, Date fecha, String codCajero) throws BaseDeDatosExcepcion, ConexionExcepcion {
		if (logger.isDebugEnabled()) {
			logger.debug("getTransacciones(int, int, Date, String) - start");
		}

		Vector<SavedTransaccion> result = new Vector<SavedTransaccion>();
		String fechaStr = getFechaSQL(fecha);
		Vector<Integer> trans = BaseDeDatosVenta.obtenerTransacciones(tda, codCajero, caja, fechaStr);
		for (Iterator<Integer> i = trans.iterator(); i.hasNext();) {
