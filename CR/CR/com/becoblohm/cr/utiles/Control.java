...
import com.becoblohm.cr.gui.DesbloqueoCaja;
import com.becoblohm.cr.gui.LoginDeUsuario;
...
public class Control {
	...
	/**
	 * 2do Formato para las fechas. Casos de registros en la BD 
	 */
	public final static SimpleDateFormat FECHA_FORMAT = new SimpleDateFormat("yyyy-MM-dd");
	
	/**
	 * Formato para montos: #,##0.00
	 */
	...
	public static boolean accesoValido(boolean autorizarFuncion, boolean paraDesbloquear) throws UsuarioExcepcion, MaquinaDeEstadoExcepcion, ConexionExcepcion, ExcepcionCr {
		...
		if (valores.size() > 0) {
			...
			if (entradaManual){
				...
				//if (autorizarFuncion)
					valido = MaquinaDeEstado.validarUsuario(login, clave, autorizarFuncion);
				//else
			...
			if (valido) {
				...
				if (autorizarFuncion)
					...
					if(Sesion.getCaja().getFechaUltRepZ()==null || fechaActual.after(fechaReporte)) {
						...
						ManejadorReportesFactory.getInstance().imprimirAperturaCajero(Sesion.getUsuarioActivo());
					} else {
	...
	* Fecha: agosto 2011
	*/
	public static ArrayList<Object> codigoValido(String codigo) {
	...
	* Fecha: agosto 2011
	*/
	public static ArrayList<Object> codigoValido(String codigo, String captura) {
		//resultado se parametriza ´Object´ dado que contiene objetos de tipo Integer y String
	...
	public static String completarCodigo(String codigo){
		...
		} else xCodigo = new StringBuffer(codigo.replaceAll(" ", "0"));
	...
	/**
	 * Remueve de la cadena todos los caracteres que no esten en la lista
	 *
	 * @param src
	 * @param lstDejar
	 *            lista de caracteres que se van a dejar
	 * @return String
	 */
	public static String removerCaracteresNoListados(String src, String lstDejar) {
		if (src == null) {
			return null;
		} else if (src.length() == 0) {
			return src;
		}
		StringBuffer strBuffer = new StringBuffer(src.length());
		char[] chrSrc = src.toCharArray();
		for (int i = 0; i < chrSrc.length; i++) {
			char c = chrSrc[i];
			if (lstDejar.indexOf(c) >= 0) {
				strBuffer.append(c);
			}
		}
	...
	/**
	 * Formatea el monto indicado aplicando el formato indicado por {@link #df}
	 *
	 * @param monto
	 * @return String
	 * @see #df
	...
	/**
	 * Convierte un objeto Number a Integer (a menos que ya lo sea)
	 *
	 * @param _number
	 *            numero a convertir
	 *
	 * @return Integer
	 */
	public static Integer number2Integer(Number _number) {
		return (Integer) ((_number instanceof Integer || _number == null) ? _number
				: new Integer(_number.intValue()));
	}
	
	/**
	 * Convierte un objeto Number a BigDecimal (a menos que ya lo sea)
	 *
	 * @param _number
	 *            numero a convertir
	 *
	 * @return BigDecimal
	 */
	public static BigDecimal number2BigDecimal(Number _number) {
		return (BigDecimal) ((_number instanceof BigDecimal || _number == null) ? _number
				: new BigDecimal(_number.doubleValue()));
	}
}
