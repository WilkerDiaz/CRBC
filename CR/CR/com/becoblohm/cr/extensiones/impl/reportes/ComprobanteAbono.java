...
public class ComprobanteAbono extends Reporte {
	...
	*/
	private static void crearEncabezado(Apartado apartado) {
		...
		Sesion.crPrinterOperations.activarFuenteImpresionNegrita();
		Sesion.crPrinterOperations.alinear(1);
		Sesion.crPrinterOperations.enviarString("A B O N O\n");
		Sesion.crPrinterOperations.initializarPrinter();
		Sesion.crPrinterOperations.activarDocumentoNomal();
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
	...
	*/
	private static void crearEncabezado(ListaRegalos lista) {
		...
		Sesion.crPrinterOperations.activarFuenteImpresionNegrita();
		Sesion.crPrinterOperations.alinear(1);
		Sesion.crPrinterOperations.enviarString("ABONO LISTA DE REGALOS\n");
		Sesion.crPrinterOperations.initializarPrinter();
		Sesion.crPrinterOperations.activarDocumentoNomal();
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
		if (lista.getCliente()!=null && lista.getCliente().getCodCliente()!=null) {
			...
			String cliente = "Cliente: ";
			String direccion = "Dirección: ";
			Vector<String> lineasNombre = dividirEnLineas(columnasImpresora - 3 - cliente.length(),lista.getCliente().getNombreCompleto());
			Vector<String> lineasDireccion = dividirEnLineas(columnasImpresora - 3 - direccion.length(),lista.getCliente().getDireccion());
			String RIF = "CI/RIF: " + lista.getCliente().getCodCliente();
	 ...
	 * @param lineas Vector de las lineas actuales de la factura.
	 */
	private static void crearDetalle(Apartado apartado) {
		if (logger.isDebugEnabled()) {
			logger.debug("crearDetalle(Apartado) - start");
		}
		...
		Sesion.crPrinterOperations.enviarString(justificar("Apartado:" + apartado.getNumServicio(), "Apertura: " + fechaTrans.format(apartado.getFechaServicio()), columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Abono Nro." + numAbonoRealizado, "Cajero: " + ((Abono)apartado.getAbonos().lastElement()).getCodCajero(),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Fecha: " + fechaTrans.format(((Abono)apartado.getAbonos().lastElement()).getFechaAbono()),
						"Hora: " + horaTrans.format(((Abono)apartado.getAbonos().lastElement()).getHoraInicia()),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Total Apartado",precio.format(apartado.consultarMontoServ()),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Abonos Realizados","(-)" + precio.format(apartado.montoAbonos()-montoAbonoActual),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(alinearDerecha(crearLineaDeDivision(17),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Saldo Anterior",precio.format(apartado.consultarMontoServ()-apartado.montoAbonos()+montoAbonoActual),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Abono Actual","(-)" + precio.format(montoAbonoActual),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(alinearDerecha(crearLineaDeDivision(17),columnasImpresora)+"\n");
		...
		if (logger.isDebugEnabled()) {
			logger.debug("crearDetalle(Apartado) - end");
		}
	}
	...
	*/
	private static void crearDetalle(ListaRegalos lista) {
		...
		Sesion.crPrinterOperations.enviarString(justificar("Lista de Regalos:" + lista.getNumServicio(), "Apertura: " + fechaTrans.format(lista.getFechaApertura()), columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Transaccion No." + numAbonoRealizado, "Cajero: " + lista.getCodCajero(),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Fecha: " + fechaTrans.format(lista.getFechaServicio()),
						"Hora: " + horaTrans.format(lista.getFechaServicio()),columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
		...
		for(int i=0;i<detalleAbonos.size();i++){
			if(((Abono)detalleAbonos.get(i)).getTipoTransAbono()=='A'){
				...
				String tipoAbono="ABONO PARCIAL";
				Sesion.crPrinterOperations.enviarString(justificar(codProd,descProd,columnasImpresoraFiscal));
				Sesion.crPrinterOperations.enviarString(justificar(tipoAbono,precio.format(montoAbono),columnasImpresoraFiscal));
			}
			else if(((Abono)detalleAbonos.get(i)).getTipoTransAbono()=='T'){
				double montoAbono = ((Abono)detalleAbonos.get(i)).getMontoBase();
				String codProd = ((Abono)detalleAbonos.get(i)).getProducto().getCodProducto();
				String descProd = ((Abono)detalleAbonos.get(i)).getProducto().getDescripcionCorta();
				String tipoAbono="ABONO TOTAL";
				Sesion.crPrinterOperations.enviarString(justificar(codProd,descProd,columnasImpresoraFiscal));
				Sesion.crPrinterOperations.enviarString(justificar(tipoAbono,precio.format(montoAbono),columnasImpresoraFiscal));
			}
			else if(((Abono)detalleAbonos.get(i)).getTipoTransAbono()=='L'){
				double montoAbono = ((Abono)detalleAbonos.get(i)).getMontoBase();
				String codProd = "000000000000";
				String descProd = "ABONO A LISTA";
				String tipoAbono="ABONO A LISTA";
				Sesion.crPrinterOperations.enviarString(justificar(codProd,descProd,columnasImpresoraFiscal));
				Sesion.crPrinterOperations.enviarString(justificar(tipoAbono,precio.format(montoAbono),columnasImpresoraFiscal));
			}
		}
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString(justificar("Monto Abonos:",precio.format(lista.getMontoAbonos()),columnasImpresoraFiscal));
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
		Sesion.crPrinterOperations.enviarString("No se efectuaran devoluciones del monto abonado.");
		Sesion.crPrinterOperations.activarFuenteImpresionNegrita();
		Sesion.crPrinterOperations.alinear(1);
		Sesion.crPrinterOperations.enviarString("COMPROBANTE NO FISCAL\n");
	...
	*/
	...
	private static void crearDetalleFiscal(Apartado apartado) {
		...
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(crearLineaDeDivision(columnasImpresoraFiscal), 0);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(justificar("Total Apartado",precio.format(apartado.consultarMontoServ()),columnasImpresoraFiscal), 0);
	...
	*/
	private static void crearDetalleFiscal(ListaRegalos lista) {
		...
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(crearLineaDeDivision(columnasImpresoraFiscal), 0);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("No se efectuaran devoluciones del monto",0);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("abonado.",0);
	...
	public static void imprimirComprobante(Apartado apartado) {
		...
		if(Sesion.impresoraFiscal) {
			...
			try {
				...
				MensajesVentanas.aviso("Impresión de Comprobante de Abono\nIntroduzca el documento en la impresora");
