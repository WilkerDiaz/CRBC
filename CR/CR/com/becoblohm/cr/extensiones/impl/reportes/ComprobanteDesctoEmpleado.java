 ...
 */
public class ComprobanteDesctoEmpleado extends Reporte {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(ComprobanteDesctoEmpleado.class);
	...
	* Fecha: agosto 2011
	*/
	private static void crearEncabezado(Cliente cte) {
		if (logger.isDebugEnabled()) {
			logger.debug("crearEncabezado(Cliente) - start");
		}
		...
		Sesion.crPrinterOperations.activarFuenteImpresionNegrita();
		Sesion.crPrinterOperations.alinear(1);
		Sesion.crPrinterOperations.enviarString("DESCUENTO A COLABORADOR\n");
		Sesion.crPrinterOperations.initializarPrinter();
		Sesion.crPrinterOperations.activarDocumentoNomal();
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
		...
		if (cte.getCodCliente()!=null) {
			...
			String cliente = "Colaborador: ";
			String numFicha = "Código: " + cte.getNumFicha();
			...
			Sesion.crPrinterOperations.enviarString(numFicha +"\n");
		...
		if (logger.isDebugEnabled()) {
			logger.debug("crearEncabezado(Cliente) - end");
		}
	}
	...
	private static void crearPieDePagina(Cliente cte, int numCopia, String autorizante) {
		...
		if (cte.getCodCliente()==null) {
			Sesion.crPrinterOperations.enviarString("Nombre Colaborador:  _____________________"+"\n\n");
			Sesion.crPrinterOperations.enviarString("Código Colaborador:  _____________________"+"\n\n");
		}
		Sesion.crPrinterOperations.enviarString("Firma del Colaborador:____________________"+"\n\n");
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora));
	...
	* Fecha: agosto 2011
	*/
	private static void crearEncabezadoFiscal(Cliente cte) {
		if (logger.isDebugEnabled()) {
			logger.debug("crearEncabezadoFiscal(Cliente) - start");
		}
		...
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("DESCTO. COLABORADOR", 1);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(crearLineaDeDivision(columnasImpresoraFiscal), 0);
		...
		if (cte.getCodCliente()!=null) {
			...
			Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(numFicha + cte.getNumFicha(), 0);
		...
		if (logger.isDebugEnabled()) {
			logger.debug("crearEncabezadoFiscal(Cliente) - end");
		}
	}
	...
	*/
	private static void crearPieDePaginaFiscal(Cliente cte, int numCopia, String autorizante) {
		...
		if (cte.getCodCliente()==null) {
			Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("Nombre Colaborador:_____________________", 0);
			Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("Código Colaborador:_____________________", 0);
		}
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(" ", 0);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("Firma Colaborador: _____________________", 0);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(crearLineaDeDivision(columnasImpresoraFiscal), 0);
		if (numCopia==1)
	 ...
	 * 	Realiza la impresion del Comprobante de Descuento a Empleado
	 * @param cte
	 ...
	 * @param numCopia
	 */
	public static void imprimirComprobate(Cliente cte, String codAutorizante, double monto, int numCopias) {
		if (logger.isDebugEnabled()) {
			logger
					.debug("imprimirComprobate(Cliente, String, double, int) - start");
		}
		...
		if(Sesion.impresoraFiscal) {
			for (int i=0; i<numCopias; i++) {
				...
				try {
					...
					if ((i+1) == 1)
						MensajesVentanas.aviso("Impresión de Comprobante de Descto. a Empleado (Original)\nIntroduzca el documento en la impresora");
					else
						MensajesVentanas.aviso("Impresión de Comprobante de Descto. a Empleado (Copia " + (i+1) + ")\nIntroduzca el documento en la impresora");
					
					Sesion.crFiscalPrinterOperations.abrirDocumentoNoFiscal();
					crearEncabezadoFiscal(cte);
					...
					logger
							.error(
									"imprimirComprobate(Cliente, String, double, int)",
									e);
				}
			}
		} else {
		...
		if (logger.isDebugEnabled()) {
			logger
					.debug("imprimirComprobate(Cliente, String, double, int) - end");
		}
	}
