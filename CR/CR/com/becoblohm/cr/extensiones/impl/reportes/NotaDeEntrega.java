 ...
 */
public class NotaDeEntrega extends Reporte {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(NotaDeEntrega.class);
	...
	private static float artAEntregar;
	...
	* Fecha: agosto 2011
	*/
	private static void crearEncabezado(Venta ventaActual, int numServ, String dirEntrega) {
		if (logger.isDebugEnabled()) {
			logger.debug("crearEncabezado(Venta, int, String) - start");
		}
...
//			Vector nuevasLineasDireccion = dividirEnLineas(columnasImpresora,lineaDeDireccion);
...
//			logger.error("crearEncabezado(Venta, int, String)", e);
...
//		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
		...
		if (ventaActual.getCliente().getCodCliente()!=null) {
			...
			String direccion = "Dirección Entrega: ";
			Vector<String> lineasNombre = dividirEnLineas(columnasImpresora - cliente.length(),ventaActual.getCliente().getNombreCompleto());
			Vector<String> lineasDireccion = dividirEnLineas(columnasImpresora - direccion.length(),dirEntrega);
			String RIF = null;
			try {
				if(InitCR.preferenciasCR.getConfigStringForParameter("sistema","estiloFactura").equals("CR")){
			...
			//Se envia la dirección del cliente
			...
			//Se envia la fecha de la entrega
		...
		Sesion.crPrinterOperations.enviarString(justificar("Tr.Asociada: " + ventaActual.getNumTransaccion(), "Entrega Nro: " + numServicio.format(numServ), columnasImpresora)+"\n");
	
		Sesion.crPrinterOperations.activarFuenteImpresionNegrita();
		Sesion.crPrinterOperations.alinear(1);
		Sesion.crPrinterOperations.enviarString("NOTA DE ENTREGA\n");
		Sesion.crPrinterOperations.initializarPrinter();
		Sesion.crPrinterOperations.activarDocumentoNomal();
		Sesion.crPrinterOperations.alinear(0);

		if (logger.isDebugEnabled()) {
			logger.debug("crearEncabezado(Venta, int, String) - end");
		}
	}
		...
		for (int i=0; i<ventaActual.getDetallesTransaccion().size(); i++) {
			...
			if (((DetalleTransaccion)ventaActual.getDetallesTransaccion().elementAt(i)).getTipoEntrega().equals(Sesion.ENTREGA_DOMICILIO)) {
				...
				Sesion.crPrinterOperations.enviarString(justificar(alinearDerecha(codProd,longitudCodigo) + " " 
								+ detalleActual.getProducto().getDescripcionCorta(), "1 " + /*detalleActual.getProducto().getAbreviadoUnidadVenta()*/
								alinearDerecha(cantidad.format(detalleActual.getCantidad()),9),columnasImpresora)+"\n");
...
//								+ "\n");
				artAEntregar += detalleActual.getCantidad();
			}
		}
		Sesion.crPrinterOperations.enviarString(crearLineaDeDivision(columnasImpresora)+"\n");
	...
	private static void crearPieDePagina(int numDocumento) {
		...
		Sesion.crPrinterOperations.enviarString("   Total Art.: " + cant.format(artAEntregar) + "\n");
		
		Sesion.crPrinterOperations.enviarString("\nFirma Autorizada:    _____________________"+"\n\n");
		Sesion.crPrinterOperations.enviarString("Firma del Cliente:   _____________________"+"\n\n");
		Sesion.crPrinterOperations.enviarString("Firma Transportista: _____________________"+"\n");
	...
	* Fecha: agosto 2011
	*/
	private static void crearEncabezadoFiscal(Venta ventaActual, int numServ, String dirEntrega) {
		if (logger.isDebugEnabled()) {
			logger.debug("crearEncabezadoFiscal(Venta, int, String) - start");
		}
...
//		
//		if (!lineaDeDireccion.equalsIgnoreCase("")) {
//			Vector nuevasLineasDireccion = dividirEnLineas(columnasImpresoraFiscal,lineaDeDireccion);
//			for (int i=0; i<nuevasLineasDireccion.size(); i++)
...
//		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(crearLineaDeDivision(columnasImpresoraFiscal),0);
		...
		if (ventaActual.getCliente().getCodCliente()!=null) {
			...
			Vector<String> lineasDireccion = dividirEnLineas(columnasImpresoraFiscal - direccion.length(),dirEntrega);
			...
			Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("Fecha de Entrega: __________________ ",0);
		...
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("Entrega Nro: " + numServicio.format(numServ),0);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("NOTA DE ENTREGA",1);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(crearLineaDeDivision(columnasImpresoraFiscal),0);	

		if (logger.isDebugEnabled()) {
			logger.debug("crearEncabezadoFiscal(Venta, int, String) - end");
		}
	}
		...
		for (int i=0; i<ventaActual.getDetallesTransaccion().size(); i++) {
			...
			if (((DetalleTransaccion)ventaActual.getDetallesTransaccion().elementAt(i)).getTipoEntrega().equals(Sesion.ENTREGA_DOMICILIO)) {
				...
				Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(justificar(alinearDerecha(codProd,longitudCodigo) + " " 
													+ detalleActual.getProducto().getDescripcionCorta(), /*"1 " + detalleActual.getProducto().getAbreviadoUnidadVenta()*/
													alinearDerecha(cantidad.format(detalleActual.getCantidad()),9),columnasImpresoraFiscal),0);
				if (!detalleActual.getCondicionVenta().equals(Sesion.condicionNormal)) {
					// Si se ha aplicado alguna rebaja (Promocion, Empaque, Cambio de Precio) se especifica
...
//								+ alinearDerecha(precio.format(detalleActual.getCantidad() * detalleActual.getPrecioFinal()), 16),0);
	...
	private static void crearPieDePaginaFiscal(int numDocumento) {
		...
		// Linea del SubTotal
		DecimalFormat cant = new DecimalFormat("#0.00");
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("   Total Art.: " + cant.format(artAEntregar),0);
		...
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal(" ",0);
		Sesion.crFiscalPrinterOperations.enviarLineaNoFiscal("Firma Transportista: ___________________",0);
	 ...
	 */
	public static void imprimirComprobante(Venta ventaActual, int numServ, String dirEntrega) {
		if (logger.isDebugEnabled()) {
			logger.debug("imprimirComprobante(Venta, int, String) - start");
		}

		//Se envía a imprimir la nota de entrega a domicilio tantas veces como lo indiquen las variables correspondientes
		for (int i=0; i < Sesion.NRO_NOTAS_ENTREGA; i++) {
			if(Sesion.impresoraFiscal) {
				Sesion.crFiscalPrinterOperations.activarSlip();
				try {
					Sesion.crFiscalPrinterOperations.commit();
					MensajesVentanas.aviso("Impresión de Nota de Entrega (Copia " + (i+1) + ")\nIntroduzca el documento en la impresora");
					// Cadenas de secciones de la factura
					Sesion.crFiscalPrinterOperations.abrirDocumentoNoFiscal();
					crearEncabezadoFiscal(ventaActual, numServ, dirEntrega);
					...
					logger.error("imprimirComprobante(Venta, int, String)", e);
				}
			} else {
				Sesion.crPrinterOperations.initializarPrinter();
				Sesion.crPrinterOperations.activarDocumentoNomal();
				...
				try {
					...
					Sesion.crPrinterOperations.initializarPrinter();
					Sesion.crPrinterOperations.activarDocumentoNomal();
					crearEncabezado(ventaActual, numServ, dirEntrega);
		...
		if (logger.isDebugEnabled()) {
			logger.debug("imprimirComprobante(Venta, int, String) - end");
		}
	}
}
