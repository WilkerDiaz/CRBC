 ...
 */
public class FacturacionPrincipal extends JDialog implements ActionListener, MouseListener, KeyListener, DocumentListener, ScanerSwitch {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(FacturacionPrincipal.class);  //  @jve:dl-index=0:
	...
	private ActualizadorPrecios ap = null;  //  @jve:decl-index=0:
	 ...
	 */
	public FacturacionPrincipal(JFrame frame) {
		super(frame, "CR ver. " + CR.version + " - Facturaci\u00f3n", true);
		inicializando = true;
		initialize();
	...
	private javax.swing.JButton getJButton2() {
		...
		if(jButton2 == null) {
			...
			try {	
				if (CR.meVenta.getVenta().getPagos().size()==0) jButton2.setEnabled(false);
	...
	private javax.swing.JButton getJButton4() {
		...
		jButton4.setEnabled(!CR.meVenta.contienePagosBeco());
	...
	*/
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (!ejecutandoEvento) {
			...
			else if(e.getSource().equals(jButton4) && this.jButton4.isEnabled()){
				ejecutandoEvento = true;
				this.actualizarTipoEntregaDetalles();
				try {
					rebajas = null;
			...
			//Mapeo de F4 en eventos de Mouse
				...
				try {				
					if(CR.meVenta.getVenta() == null) {
						...
						double totalAPagar = CR.meVenta.consultarMontoTrans(true);
						//fin
						...
						/*else if (!(CR.meVenta.isPantallaVenta()))
						
						{
							
						}*/ else {
							try {
								Vector<Object> pagos = ManejoPagosFactory.getInstance().realizarPago(totalAPagar, CR.meVenta.getVenta().getPagos(), CR.meVenta.getVenta().getCliente(), CR.meVenta.getVenta().getNumTransaccion());
								if (((Boolean)pagos.elementAt(2)).booleanValue()) {
									...
									//Linea movida fuera del if por PROMOCIONES, debe actualizarse el resto por
									//por si existian donaciones
									//this.repintarFactura();
								...
								if (monto >= (CR.meVenta.getVenta().obtenerMontoPagado()-totalAPagar)) {
									//Se finaliza la venta
									CR.meVenta.finalizarVenta();
					...
					if (casoConsulta!=null) {
						try {
							if (new Integer(casoConsulta).intValue()<8) {
								...
								//Cambio por crm. Preguntar por Consultar si es Contribuyente o no
								RegistroCliente registro = factory.getInstance();
								registro.MostrarPantallaCliente(true,0);
								this.repintarFactura();
							}
						} catch (ProductoExcepcion e1) {
							logger.error("mouseClicked(MouseEvent)" + e1.getMensaje());
			...
			//Mapeo de F6 en eventos de Mouse
				...
				clienteEnEspera = new ClientesEnEspera();
				MensajesVentanas.centrarVentanaDialogo(clienteEnEspera);
				clienteEnEspera = null;
				this.repintarFactura();
				if (logger.isDebugEnabled()) {
			...
			//Mapeo de F7 en eventos de Mouse
				...
				MensajesVentanas.centrarVentanaDialogo(servicios);
				servicios = null;
				this.repintarFactura();
				this.comprobarLimiteEntrega();
				if (logger.isDebugEnabled()) {
			...
			//Mapeo de F8 en Eventos de Mouse
				...
				jButton2.setEnabled(!CR.meVenta.contienePagosBeco());
				this.repintarFactura();
					...
					if(MensajesVentanas.preguntarSiNo("Se cerrará la sesión actual.\nCualquier venta activa será anulada.\n¿Confirma que desea cerrar la sesion?")==0) {
						if (validarClaveAlSalir) {
							...
							if (vp.isClaveCorrecta()) {
								...
								/*if(MaquinaDeEstado.cerrarSesion()) {
										dispose();
									} else {
										this.repintarFactura();
									}*/
				...
				try {
					if(CR.meVenta.getVenta() == null) {
						...
						if(CR.meVenta.getVenta().verificarTieneEntregaDom() && CR.meVenta.getVenta().getCliente().getCodCliente() == null) {
							...
							try {
								Vector<Object> pagoRealizado = ManejoPagosFactory.getInstance().realizarNuevoPago(totalAPagar ,CR.meVenta.getVenta().getPagos(), CR.meVenta.getVenta().getCliente());
...
/************************************************/
				...
				if(e.getClickCount()==2){
					...
					actualizadorPrecios.abrirVentanaCondicionesVenta(((DetalleTransaccion)(CR.meVenta.getVenta().getDetallesTransaccion().elementAt(jTable.getSelectedRow()))).getCondicionesVentaPromociones());
				} //Caso agregado por CENTROBECO porque fallaba al hacerlo con el mouse
			  else if (e.getSource() == getJTable()) {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (!ejecutandoEvento) {
			...
			//Mapeo de F3
			else if(e.getKeyCode() == KeyEvent.VK_F3 && this.jButton4.isEnabled()){
				ejecutandoEvento = true;
				this.actualizarTipoEntregaDetalles();
				ejecutado = true;
				try {
			...
			//Mapeo de F4
				...
				try{								
					...
					logger.error("keyPressed(KeyEvent)", f1);
			...
			//Mapeo de F7
				...
				servicios = null;
				if (logger.isDebugEnabled()) {
					logger.debug("keyPressed(KeyEvent) - F7");
				}
			}
			...
			//Al presionar la tecla 'D' muestra la informacion de las condiciones de venta
		...
		try{
			...
			if (CR.meVenta.getVenta() == null) {
				...
				if (CR.meVenta.getVenta().getCliente() == null || 
						...
						if(CR.meVenta.getVenta().getDescuentoCorporativoAplicado()!=0.0) { //Actualizado por beco por modulo de promociones
						...
						if((((CR.meVenta.getVenta().getCliente().getTipoCliente() == Sesion.COLABORADOR)&&
								(CR.meVenta.getVenta().getCliente().getEstadoColaborador() == Sesion.ACTIVO)) && 
								!Sesion.permitirRebajasAempleados) || 
								CR.meVenta.getVenta().getDescuentoCorporativoAplicado()!=0.0) { //Actualizado por beco por modulo de promociones
				...
				if (CR.meVenta.getVenta().consultarMontoTrans() - CR.meVenta.getVenta().obtenerMontoPagado()>=0)
					jLabel9.setText("RESTO: " + df.format(CR.meVenta.getVenta().consultarMontoTrans() - CR.meVenta.getVenta().obtenerMontoPagado()+ap.getDonacionesVenta()));
				else
					jLabel9.setText("VUELTO: " + df.format(CR.meVenta.getVenta().obtenerMontoPagado()-CR.meVenta.getVenta().consultarMontoTrans()));
	...
	public void leerEscaner(final String codigoScaner) {
		...
		try {
			...
			logger.error("leerEscaner(String)" + e.getMessage());
		}
		try {
	...
	*/
	public void lecturaEscaner(String codigoScaner) {
		...
		if (isPermitirEscaner()) {
			try {
				...
				logger.error("lecturaEscaner(String)" + e1.getMessage());
				...
				setPermitirEscaner(true);
			}
			
			repintarFactura();
		} else {
			logger.error("Entro aqui, tiene seteado permitirEscaner es nop:" +permitirEscaner+ "");
			CR.beep();
		}
		if (logger.isDebugEnabled()) {
	...
	class TablaFact extends JTable {
		...
		public void tableChanged(TableModelEvent e){
			...
			if ((e.getColumn() == FacturacionPrincipal.column) && (e.getType() == TableModelEvent.UPDATE)) {
				actualizarTipoEntregaDetalles();
			}
			if (getRowCount() > 0) {
				changeSelection(getRowCount() - 1, FacturacionPrincipal.column, false, false);
			}
			if (!inicializando) {
	...
	/**
	 * Crea una instancia del actualizador de precios para realizar alguna 
	 * funcion del modulo de promociones
	 *
	 */
	public void iniciarActualizadorPrecios(){
		...
		ActualizadorPreciosFactory  factory = new ActualizadorPreciosFactory();
		ap = factory.getActualizadorPreciosInstance();		
		if (logger.isDebugEnabled()) {
