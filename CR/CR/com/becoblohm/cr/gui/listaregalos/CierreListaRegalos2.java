 ...
 */
public class CierreListaRegalos2 extends JDialog implements ActionListener, KeyListener, DocumentListener, ScanerSwitch {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(CierreListaRegalos2.class);
	...
	private javax.swing.JTable jTableCierre2 = null;
	...
	private ModeloTablaLR tablaCierre = new ModeloTablaLR(7);
	...
	public CierreListaRegalos2(FacturacionPrincipal principal,char estadoLista) throws ConexionExcepcion, ExcepcionCr {
		super(MensajesVentanas.ventanaActiva);
		this.principal = principal;
		initialize();
		if(CR.meServ.getListaRegalos().getVentaParcialDeCierre2()!=null){
			CR.meVenta.setVenta(CR.meServ.getListaRegalos().getVentaParcialDeCierre2());
			estado = FACTURANDO;
		}else{
	...
	*/
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource().equals(jButton1)){
			int fila = -1;
			JTable tabla = null;
			if(jTableCierre2.getSelectedRow() > -1){
				fila = jTableCierre2.getSelectedRow();
				tabla = jTableCierre2;
			}
			...
			if(fila > -1 /*&& !jTableFactura.getValueAt(fila,0).equals("000000000000")*/){
				...
				if(cantidadStr != null){
					...
					DetalleServicio detalle = (DetalleServicio)CR.meServ.getListaRegalos().getDetalleNoVendidos().get(fila);
					...
					if(cantidad < 1)
						...
						try {
							...
							try{
								...
								if(cantActual+cantidad>cantRestantes)
									MensajesVentanas.aviso("No puede agregar más productos \nde los abonados");
								else {
									if(cantidad > 1){
										CR.meVenta.ingresarLineaProductoVentaLR(codprod,Sesion.capturaTeclado,2);
		...
		/* Botón Vender Todos */
			...
			Vector<DetalleServicio> noVendidos = CR.meServ.getListaRegalos().getDetalleNoVendidos();
			if(noVendidos.size()==0)
				MensajesVentanas.aviso("No hay productos para agregar a factura");
			else
				for(int i=0;i<noVendidos.size();i++){
					DetalleServicio detalle = (DetalleServicio)noVendidos.get(i);
					try {
						int cantActual = 0;
						...
						if(detalle.getCantRestantes()<cantActual){ 
							CR.meVenta.ingresarLineaProductoVentaLR(detalle.getProducto().getCodProducto(),detalle.getTipoCaptura(),2);
							if(detalle.getCantRestantes()-cantActual>1)
								CR.meVenta.cambiarCantidad(detalle.getCantRestantes()-cantActual-1);
							estado = FACTURANDO;
						}
					} catch (ProductoExcepcion e1) {
						try {
							CR.meVenta.ingresarLineaProductoVentaLR(detalle.getProducto().getCodProducto(),detalle.getTipoCaptura(),2);
							if(detalle.getCantRestantes()>1)
								CR.meVenta.cambiarCantidad(detalle.getCantRestantes()-1);
							estado = FACTURANDO;
		...
		/* Botón Continuar */
			...
			try {
				if(CR.meVenta.getVenta()==null)
					CR.meVenta.crearVenta();
				CR.meServ.getListaRegalos().guardarVentaParcialCierre(CR.meVenta.getVenta(),2);
				CierreListaRegalos3 clr3 = new CierreListaRegalos3(principal,ListaRegalos.CIERRE_LISTA);
				dispose();
				MensajesVentanas.centrarVentanaDialogo(clr3);
			} catch (ConexionExcepcion e1) {
				e1.printStackTrace();
			} catch (ExcepcionCr e1) {
				e1.printStackTrace();
...
//			}
	...
	private javax.swing.JPanel getJPanel2() {
		...
		if(jPanel2 == null) {
			...
			jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Productos No Vendidos de Lista de Regalos No. "+numLista+": ", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Dialog", java.awt.Font.BOLD, 11), java.awt.Color.black));
	...
	private javax.swing.JScrollPane getJScrollPane() {
		if(jScrollPane == null) {
			...
			jScrollPane.setViewportView(getJTableCierre2());
	 ...
	 */
	private javax.swing.JTable getJTableCierre2() {
		if(jTableCierre2 == null) {
			jTableCierre2 = new javax.swing.JTable(tablaCierre);
			jTableCierre2.getTableHeader().setReorderingAllowed(false) ;
			jTableCierre2.getColumnModel().getColumn(0).setPreferredWidth(65);
			jTableCierre2.getColumnModel().getColumn(1).setPreferredWidth(175);
			jTableCierre2.getColumnModel().getColumn(2).setPreferredWidth(60);
			jTableCierre2.getColumnModel().getColumn(3).setPreferredWidth(30);
			jTableCierre2.getColumnModel().getColumn(4).setPreferredWidth(30);
			jTableCierre2.getColumnModel().getColumn(5).setPreferredWidth(60);
			jTableCierre2.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
			jTableCierre2.setBackground(new java.awt.Color(242,242,238));
			jTableCierre2.addMouseListener(new MouseAdapter() {
				public void mouseClicked(MouseEvent e) {
					if (e.getClickCount() == 2) {
						int fila = jTableCierre2.getSelectedRow();
	...
	private javax.swing.JPanel getJPanel5() {
		if(jPanel5 == null) {
			...
			jPanel5.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Productos seleccionados para facturar:", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Dialog", java.awt.Font.BOLD, 11), java.awt.Color.black));
	...
	private javax.swing.JTable getJTableFactura() {
		if(jTableFactura == null) {
			...
			jTableFactura.addMouseListener(new MouseAdapter(){
				public void mouseClicked(MouseEvent e) {				
					getJTableCierre2().getSelectionModel().clearSelection();
				}
			});
		}
	...
	private void repintarPantalla(){
		...
		try{
			...
			tablaCierre.llenarTablaCierre2();
			...
			if(estado == FACTURANDO) {
				...
				try{
					montoBase = CR.meVenta.getVenta().getMontoBase()+CR.meServ.getListaRegalos().getVentaParcialDeCierre1().getMontoBase()+CR.meServ.getListaRegalos().getVentaParcialDeCierre3().getMontoBase();
					montoImpuesto = CR.meVenta.getVenta().getMontoImpuesto()+CR.meServ.getListaRegalos().getVentaParcialDeCierre1().getMontoImpuesto()+CR.meServ.getListaRegalos().getVentaParcialDeCierre3().getMontoImpuesto();
				}catch(Exception e){
					montoBase = CR.meVenta.getVenta().getMontoBase()+CR.meServ.getListaRegalos().getVentaParcialDeCierre1().getMontoBase();
					montoImpuesto = CR.meVenta.getVenta().getMontoImpuesto()+CR.meServ.getListaRegalos().getVentaParcialDeCierre1().getMontoImpuesto();
				}
				...
				getJScrollPane().setViewportView(getJTableCierre2());
	...
	public void insertUpdate(DocumentEvent e) {
		//jperez12-04-2012 Se elimino la captura de escaner desde la pantalla de cierre2
		/*if (lector == null)
	...
	*/
		...
		try {
			...
			switch (tipoCodigo){
				case Control.PRODUCTO:
					CR.meVenta.ingresarLineaProductoVentaLR(codigo, Sesion.capturaEscaner,2);
					estado = FACTURANDO;
					break;
				...
				case Control.CODIGO_DESCONOCIDO: 
					try{
						...
						try {
							CR.meVenta.ingresarLineaProductoVentaLR(codigoScaner, Sesion.capturaEscaner,2);
							estado = FACTURANDO;
						} catch (Exception ex2) {
							CR.meVenta.ingresarLineaProductoVentaLR(codigoScaner.substring(0,codigoScaner.length()-1),Sesion.capturaEscaner,2);
							estado = FACTURANDO;
						}
					}
					break;
	...
	private void agregarListeners(){
		...
		getJTableCierre2().addKeyListener(this);
	}
	...
	private void eliminarListeners(){
		...
		getJTableCierre2().removeKeyListener(this);
	}
	...
	private javax.swing.JLabel getJLabel9() {
		if(jLabel9 == null) {
			...
			jLabel9.setText("Abonos a Productos: ");
