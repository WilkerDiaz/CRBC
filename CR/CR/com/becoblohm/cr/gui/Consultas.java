 ...
 * Descripción : - Agregado manejo de consultas de productos y clientes, a través del método ejecutarConsulta(char tecla) la cual 
 *			distingue entre los criterios de búsquedas perdefinidos en la GUI.
 * 				- Mapeadas teclas de dirección arriba y abajo.
 *				- Eliminado los objetos jPanel3 y la jLabel1.
 *	 			- Comentarizados definición del ancho de las columnas de la tabla.
 * =============================================================================
 * Versión     : 1.1
 * Fecha       : May 12, 2004
 * Analista    : Gabriel Martinelli <gmartinelli@beco.com.ve>
 * Descripción : Agregado manejo de consultas de productos y clientes
 * =============================================================================
 * Versión     : 1
 * Fecha       : Mar 2, 2004
 ...
 */
public class Consultas extends JDialog implements ComponentListener, KeyListener, MouseListener, ActionListener {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(Consultas.class);  //  @jve:decl-index=0:
	...
	private String camposBusqueda[] = new String[]{"codproducto","codexterno","descripcioncorta","d.nombre","l.nombre","marca","modelo","referenciaproveedor","nombre","codafiliado"};//,"direccion"};  //  @jve:decl-index=0:
	...
	private String codigoSeleccionado[] = new String[2];  //  @jve:decl-index=0:
	...
	private void ejecutarConsulta(char tecla) {
		...
		int longRelleno = Control.getLONGITUD_CODIGO() - Sesion.getLongitudCodigoInterno();
		StringBuffer relleno = new StringBuffer("");
		if (casoConsulta<8) {
			// Consulta de Productos
			try {
				CR.me.consultaProducto();
				if(casoConsulta == 0){
					for (int j=0; j<longRelleno; j++)
						relleno.append('0');
				}
				if (i.intValue()>=32 && i.intValue()<=126)
					resultadoConsulta.cambiarDatosTabla(MediadorBD.buscarElemento(relleno + jTextField.getText() + String.valueOf(tecla),camposBusqueda[casoConsulta],limiteInf,1,nombresCamposBusqueda[casoConsulta]));
				else
					if ((i.intValue()==8)&&(jTextField.getText().length()>0))
						resultadoConsulta.cambiarDatosTabla(MediadorBD.buscarElemento(relleno + jTextField.getText().substring(0,jTextField.getText().length()-1),camposBusqueda[casoConsulta],limiteInf,1,nombresCamposBusqueda[casoConsulta]));
					else
						resultadoConsulta.cambiarDatosTabla(MediadorBD.buscarElemento(relleno + jTextField.getText(),camposBusqueda[casoConsulta],limiteInf,1,nombresCamposBusqueda[casoConsulta]));
	...
	private void ejecutarConsulta() {
		if (logger.isDebugEnabled()) {
			logger.debug("ejecutarConsulta() - start");
		}

		int longRelleno = Control.getLONGITUD_CODIGO() - Sesion.getLongitudCodigoInterno();
		StringBuffer relleno = new StringBuffer("");
		if (casoConsulta == 0)
			for (int i = 0; i < longRelleno; i++) {
				relleno.append("0");
			}
		if (casoConsulta<8) {
			try {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("ejecutarConsulta() - end");
		}
	}
	...
	private javax.swing.JComboBox getJComboBox() {
		...
		if(jComboBox == null) {
			...
			if(Sesion.getCaja().getEstado().equals("23")){ // Estado: Cierre de Listas de Regalos
