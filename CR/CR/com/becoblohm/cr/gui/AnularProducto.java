 ...
 * Descripción : - Modificado el atributo modeloTablaAnularProducto de la clase ModeloTabla a la clase ModeloTablaAnularProducto.
 * 				- Modificado el método anularProducto para que reciba la fila del detalle de productos a anular.
 * 				- Agregado el indicador de la cantidad de producto a anular.
 *		 		- Eliminado el mapeo de la tecla <delete> y la tecla <s>.
 * 				- Agregado el mapeo de la tecla <tabulador>.
 *		 		- Agregado el método restablecerTabla() y el método getJPanel5().
 *  * =============================================================================
 * Versión     : 1
 * Fecha       : Mar 1, 2004
 ...
 */
public class AnularProducto extends JDialog implements ComponentListener,KeyListener, MouseListener, EventListener, DocumentListener{
	/**
	...
	ModeloTabla modeloTablaAnularProducto = new ModeloTabla();
	 ...
	 */
	public AnularProducto (int filaACambiar){
		...
		try {
			...
			float cantidadProd = ((DetalleTransaccion)CR.meVenta.getVenta().getDetallesTransaccion().elementAt(filaSeleccionadaInicial)).getCantidad(); 
//			jTextField1.setText(String.valueOf(cantidadProd <= 1 ? df.format(cantidadProd) : df.format(1)));
			jTextField1.setText(df.format(cantidadProd));
		} catch (ExcepcionCr e1) {
			logger.error("AnularProducto(int)", e1);
		...
		//Mapeo de TAB sobre el componente jTable y jTextField
			...
			if(e.getSource().equals(this.getJButton1())) {
				...
				if(jTable.getSelectedRow() != -1) {
					...
					try {
						//CR.meVenta.anularProducto(new Integer(CR.meVenta.obtenerRenglones(jTextField.getText().trim().toString()).elementAt(filaSeleccionada).toString()).intValue(), Float.valueOf(CR.me.formatoNumerico(getJTextField1().getText())).floatValue());
						CR.meVenta.anularProducto(new Integer(CR.meVenta.obtenerRenglones(productoABuscar, false).elementAt(filaSeleccionada).toString()).intValue(), Float.valueOf(CR.me.formatoNumerico(getJTextField1().getText())).floatValue());
						this.ejecutado = true;
						dispose();
						try {
							finalize();
				...
				CR.inputEscaner.getDocument().removeDocumentListener(this);
				try {
					modeloTablaAnularProducto.llenarTabla(jTextField.getText().trim(), true);
