 ...
 */
public class PantallaDevolucion extends JDialog implements ActionListener, MouseListener, KeyListener, DocumentListener {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(PantallaDevolucion.class);
	 ...
	 */
	public PantallaDevolucion(int tda, int caja, int transaccion, Date fecha) {
	...
	private void initialize() {
		...
		this.setTitle("CR - Devoluci\u00f3n por Dinero");
		...
		if(e.getSource().equals(jButton6)){
			if (CR.meVenta.getDevolucion().getCliente().getCodCliente() != null){
				...
				if (CR.meVenta.getDevolucion().getDetallesTransaccion().size()>0) {
					try {					
						if(MensajesVentanas.preguntarSiNo("¿Está seguro que desea\n finalizar la devolución?")==0) {
							//Esta forma de Pago hay que sustituirla por una busqueda en la sesión. No dejar este número así					
							//Pago p = new Pago("1111111111", CR.meVenta.getDevolucion().consultarMontoTrans(true), null, null, null, 0, 0, null);
						/*	Pago p = new Pago(Sesion.FORMA_PAGO_EFECTIVO, CR.meVenta.getDevolucion().consultarMontoTrans(true), null, null, null, null, 0, null);
					...
					MensajesVentanas.aviso("No se han seleccionado productos para la devolución.");
					CR.inputEscaner.getDocument().addDocumentListener(this);
				}
			} else {
...
/************************************************/
	...
	* Se parametrizó el tipo de dato de los distintos ArrayList y se comentaron variables sin uso
	* Fecha: agosto 2011
	*/
