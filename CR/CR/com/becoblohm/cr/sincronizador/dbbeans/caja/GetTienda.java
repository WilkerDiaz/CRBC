...
public class GetTienda {
	...
	protected void initializer() {
		...
		try {
			...
			select.setCommand("SELECT DISTINCT * FROM tienda");
	 ...
	 */
	public GetTiendaRow[] getRows() {
		GetTiendaRow[] rows = new GetTiendaRow[select.getRowCount()];
		for (int i = 0; i <= select.getRowCount() - 1; i++) {
			rows[i] = new GetTiendaRow(select, i + 1);
		};
		return rows;
	}
}