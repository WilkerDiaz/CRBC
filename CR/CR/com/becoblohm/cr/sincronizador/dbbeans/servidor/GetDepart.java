...
public class GetDepart {
	...
	protected void initializer() {
		...
		try {
			...
			select.setCommand("SELECT DISTINCT * FROM SISTEMA.DEPART");
	 ...
	 */
	public GetDepartRow[] getRows() {
		GetDepartRow[] rows = new GetDepartRow[select.getRowCount()];
		for (int i = 0; i <= select.getRowCount() - 1; i++) {
			rows[i] = new GetDepartRow(select, i + 1);
		};
		return rows;
	}
}