...
public class GetModulo {
	...
	protected void initializer() {
		...
		try {
			...
			select.setCommand(
				"SELECT DISTINCT modulos.codmodulo, modulos.descripcion, modulos.regvigente, modulos.actualizacion FROM modulos");
	 ...
	 */
	public GetModuloRow[] getRows() {
		GetModuloRow[] rows = new GetModuloRow[select.getRowCount()];
		for (int i = 0; i <= select.getRowCount() - 1; i++) {
			rows[i] = new GetModuloRow(select, i + 1);
		};
		return rows;
	}
}