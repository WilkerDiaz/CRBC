...
public class GetMetodo {
	...
	protected void initializer() {
		...
		try {
			...
			select.setCommand(
				"SELECT DISTINCT metodos.codmetodo, metodos.descripcion FROM metodos");
	 ...
	 */
	public GetMetodoRow[] getRows() {
		GetMetodoRow[] rows = new GetMetodoRow[select.getRowCount()];
		for (int i = 0; i <= select.getRowCount() - 1; i++) {
			rows[i] = new GetMetodoRow(select, i + 1);
		};
		return rows;
	}
}