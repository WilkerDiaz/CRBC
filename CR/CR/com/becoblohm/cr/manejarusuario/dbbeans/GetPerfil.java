...
public class GetPerfil {
	...
	protected void initializer() {
		...
		try {
			...
			select.setCommand(
				"SELECT DISTINCT perfil.codperfil, perfil.descripcion, perfil.nivelauditoria, perfil.regvigente, perfil.actualizacion FROM perfil");
	 ...
	 */
	public GetPerfilRow[] getRows() {
		GetPerfilRow[] rows = new GetPerfilRow[select.getRowCount()];
		for (int i = 0; i <= select.getRowCount() - 1; i++) {
			rows[i] = new GetPerfilRow(select, i + 1);
		};
		return rows;
	}
}