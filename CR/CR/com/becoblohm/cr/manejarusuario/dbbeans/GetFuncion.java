...
public class GetFuncion {
	...
	protected void initializer() {
		...
		try {
			...
			select.setCommand(
				"SELECT DISTINCT funcion.codfuncion, funcion.descripcion, funcion.codmodulo, funcion.nivelauditoria, funcion.regvigente, funcion.actualizacion, funcion.reqautorizacion FROM funcion WHERE funcion.regvigente = 'S'");
	 ...
	 */
	public GetFuncionRow[] getRows() {
		GetFuncionRow[] rows = new GetFuncionRow[select.getRowCount()];
		for (int i = 0; i <= select.getRowCount() - 1; i++) {
			rows[i] = new GetFuncionRow(select, i + 1);
		};
		return rows;
	}
}