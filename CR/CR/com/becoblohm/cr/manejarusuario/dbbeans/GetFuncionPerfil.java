 ...
 * Programa   : GetFuncionPerfil.java
...
public class GetFuncionPerfil {
	...
	protected void initializer() {
		...
		try {
			...
			select.setCommand(
				"SELECT DISTINCT funcionperfil.codperfil, funcionperfil.codfuncion, funcionperfil.habilitado, funcionperfil.autorizado, funcionperfil.actualizacion, funcionperfil.codmodulo FROM funcionperfil");
	 ...
	 */
	public GetFuncionPerfilRow[] getRows() {
		GetFuncionPerfilRow[] rows =
			new GetFuncionPerfilRow[select.getRowCount()];
		for (int i = 0; i <= select.getRowCount() - 1; i++) {
			rows[i] = new GetFuncionPerfilRow(select, i + 1);
		};
		return rows;
	}
}