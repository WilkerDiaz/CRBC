 ...
 * Programa   : GetMaquinaEstado.java
...
public class GetMaquinaEstado {
	...
	protected void initializer() {
		...
		try {
			...
			select.setCommand(
				"SELECT DISTINCT maquinadeestado.edoinicial, maquinadeestado.codmetodo, maquinadeestado.edofinal FROM maquinadeestado");
	 ...
	 */
	public GetMaquinaEstadoRow[] getRows() {
		GetMaquinaEstadoRow[] rows =
			new GetMaquinaEstadoRow[select.getRowCount()];
		for (int i = 0; i <= select.getRowCount() - 1; i++) {
			rows[i] = new GetMaquinaEstadoRow(select, i + 1);
		};
		return rows;
	}
}