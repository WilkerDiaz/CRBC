/*
 * $Id: MySQLUpdateScriptDataFile.java,v 1.2 2005/03/30 17:50:03 programa4 Exp $
 * ===========================================================================
 ...
 * Actualizaciones:
 * ---------------------------------------------------------------------------
 * $Log: MySQLUpdateScriptDataFile.java,v $
 ...
 * @since 27/12/2004 @
 */
public class MySQLUpdateScriptDataFile extends MySQLScriptDataFile {
    /**
     * @since 27/12/2004
     * @param outputfile
     * @param versionMySQL
     * @throws IOException
     */
    public MySQLUpdateScriptDataFile(File outputfile, String versionMySQL)
            throws IOException {
	...
	* Sólo se parametrizó el tipo de dato contenido en los 'Map', 'Treemap' e 'Iterator'
	* Fecha: agosto 2011
	*/
    public String getSQL(String nombreTabla, Map<String,Object>[] datos) {
        ...
        for (int i = 0; i < datos.length; i++) {
            ...
            Map<String,Object> noPK = new TreeMap<String,Object>(map);
            Map<String,Object> pk = new TreeMap<String,Object>();
            Iterator<String> pkIterator = pk.keySet().iterator();
            while (pkIterator.hasNext()) {
                Object key = pkIterator.next();
                noPK.remove(key);
            }
            if (pk.size() == 0) {
                throw new UnsupportedOperationException(
                        "SE DEBE IMPLEMENTAR UNA MANERA, "
                                + "DE IDENTIFICAR LOS CAMPOS CLAVES, PARA EVITAR ACTUALIZAR TODA "
                                + "LA TABLA");
            }
            
            sqlInsert.append("UPDATE ");
            sqlInsert.append(nombreTabla);
            sqlInsert.append(" SET  " + printSetValue(noPK));
            sqlInsert.append(" WHERE " + printAndConditionValue(pk));
            sqlInsert.append(";");
